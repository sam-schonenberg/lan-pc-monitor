...
crosses its reset boundary; warning may escalate once to critical. The evaluator supports temperature, memory/VRAM
pressure, sustained CPU/GPU utilization, and temperature-gated fan-speed alerts. Utilization alerts default to off
...
Push delivery is disabled by default. Configure `Notifications:FirebaseProjectId` and
`Notifications:FirebaseServiceAccountFile`, then set `Notifications:Enabled` to `true`. The service uses the
Firebase Cloud Messaging HTTP v1 API; the service-account file remains on the monitored PC and must never be
sent to a mobile client.

The app creates a stable installation ID, obtains an FCM token, and registers it:
...
Register again whenever FCM rotates the token. The response deliberately omits the token. To sign out or stop
notifications, call `DELETE /api/v1/notifications/devices/{installationId}`. Registrations are stored under the
common application-data directory by default. Invalid/expired FCM tokens are removed automatically.
